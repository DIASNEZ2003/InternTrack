...
function AddStudent() {
  ...
  const [formData, setFormData] = useState({ first_name: '', last_name: '', username: '', password: '', confirm_password: '' });
  ...
  useEffect(() => { fetchAdminAndStudents(); }, []);
  ...
  const openModal = (mode, student = null) => {
    ...
    if (student) {
      setFormData({ 
        ...
        password: '', // Kept empty securely for edits
        confirm_password: '' 
      });
      setActiveStudentId(student.id);
    } else {
      setFormData({ first_name: '', last_name: '', username: '', password: '', confirm_password: '' });
  ...
  const handleSubmit = async (e) => {
    ...
    // Only check passwords if the user is typing a new one
    ...
    try {
      ...
      // Ensure we explicitly save the plain text password to the database if admin typed a new one
      ...
      if (modalMode === 'add') {
        ...
        const { data: authData, error: authError } = await supabase.auth.signUp({ email: `${formData.username}@student.com`, password: formData.password });
        if (authError) throw authError;
        await supabase.from('profiles').upsert([{ id: authData.user.id, ...payload }]);
      } else {
        await supabase.from('profiles').update(payload).eq('id', activeStudentId);
      }
  ...
  const filteredStudents = students.filter(student => (`${student.first_name} ${student.last_name}`.toLowerCase().includes(searchQuery.toLowerCase()) || (student.username || '').toLowerCase().includes(searchQuery.toLowerCase())));
    ...
    <div className="h-full flex flex-col p-4 sm:p-6 animate-fade-in w-full relative">
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between w-full mb-4 gap-3 p-3 rounded-xl border ${bgCard}`}>
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search student..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-9 pr-3 py-2 rounded-lg text-[12px] font-medium outline-none transition-colors ${bgInput} ${theme.ring}`} />
        </div>
        <button onClick={() => openModal('add')} className={`flex items-center gap-1.5 text-white px-4 py-2 rounded-lg font-bold text-xs shadow-lg transition-colors border border-white/10 ${theme.primary}`}>
          <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v16m8-8H4M4 12h16" /></svg> Add Student
        </button>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-max">
              ...
              <tbody className={`divide-y ${isDarkMode ? 'divide-white/5' : 'divide-gray-100'}`}>
                {filteredStudents.map((student, index) => {
                  ...
                  // NEW DISPLAY LOGIC: Shows 'Private Password' if they signed up themselves
                    ...
                    <tr key={student.id} className={`transition-colors group ${bgHover}`}>
                      ...
                      <td className="py-2.5 px-4">
                        <div className="flex items-center gap-3">
                          {student.avatar_url ? (
                            ...
                            <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 font-bold text-xs ${isDarkMode ? theme.bgLight : 'bg-gray-100'} ${theme.text}`}>{student.first_name?.charAt(0)}{student.last_name?.charAt(0)}</div>
                          )}
                          <div>
                            <p className={`text-[13px] font-bold leading-tight ${textMain}`}>{student.first_name} {student.last_name}</p>
                            <p className={`text-[11px] uppercase ${textMuted}`}>{student.department}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-2.5 px-4">
                        <span className={`text-[12px] font-medium px-2 py-1 rounded-md border ${isDarkMode ? 'bg-black/40 border-white/5 text-gray-300' : 'bg-gray-100 border-transparent text-gray-700'}`}>{student.username || 'N/A'}</span>
                      </td>
                      <td className="py-2.5 px-4">
                      ...
                      <td className="py-2.5 px-4 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => openModal('edit', student)} className={`px-2.5 py-1.5 rounded-md text-[11px] font-bold transition-colors ${btnEdit}`}>Edit</button>
                          <button onClick={() => {setStudentToDelete(student); setIsDeleteModalOpen(true);}} className={`p-1.5 rounded-md transition-colors ml-1 ${btnDelete}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
                        </div>
                      </td>
                    </tr>
                  );
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden border border-white/10 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            <div className={`px-6 py-4 flex justify-between items-center border-b ${isDarkMode ? 'border-white/10 bg-black/40' : 'border-gray-100 bg-gray-50'}`}>
              ...
              <button onClick={closeModal} className={`${textMuted} hover:text-white hover:bg-white/10 p-1.5 rounded-md`}><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
              ...
              <div className={`pt-3 flex justify-end gap-2 mt-2 border-t ${isDarkMode ? 'border-white/10' : 'border-gray-100'}`}>
                <button type="button" onClick={closeModal} className={`px-4 py-2 rounded-lg font-bold text-xs transition-colors ${isDarkMode ? 'text-gray-300 bg-white/5 hover:bg-white/10 border border-white/10' : 'text-gray-600 bg-gray-100 hover:bg-gray-200'}`}>Cancel</button>
                <button type="submit" disabled={isSubmitting} className={`px-5 py-2 rounded-lg font-bold text-xs text-white shadow-lg border border-white/10 ${theme.primary}`}>{isSubmitting ? 'Saving...' : 'Save'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className={`rounded-2xl shadow-2xl w-full max-w-[300px] flex flex-col p-6 text-center items-center border border-white/10 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <div className="w-full flex gap-2">
              ...
              <button onClick={handleDeleteConfirm} className="py-2 rounded-lg font-bold text-xs text-white shadow-lg flex-1 bg-red-600 hover:bg-red-500">Delete</button>
            </div>
          </div>
        </div>
      )}
