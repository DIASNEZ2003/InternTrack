...
function StudentReports() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // 2. Fetch ALL students in this specific department
  ...
  const handleViewDocument = async (journal) => {
    ...
    try {
      ...
      // Handle both new .json format and old .html format
  ...
  const handleDownload = async (journal) => {
    try {
      ...
      // Extract data depending on file format
      if (journal.file_name.endsWith('.json')) {
        const parsed = JSON.parse(text);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await supabase.storage.from('journals').remove([reportToDelete.file_path]);
      await supabase.from('saved_journals').delete().eq('id', reportToDelete.id);
    ...
    <div className="h-full flex flex-col p-4 sm:p-6 animate-fade-in w-full relative">
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between w-full mb-4 gap-3 p-3 rounded-xl border ${bgCard}`}>
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="text" placeholder="Search student or file name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-9 pr-3 py-2 rounded-lg text-[12px] font-medium outline-none transition-colors ${bgInput} ${theme.ring}`} />
        </div>
        <div className="flex items-center gap-2">
          <select value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)} className={`px-3 py-2 rounded-lg text-[12px] font-medium outline-none transition-colors ${selectBg} ${theme.ring}`}>
          ...
          <select value={weekFilter} onChange={(e) => setWeekFilter(e.target.value)} className={`px-3 py-2 rounded-lg text-[12px] font-medium outline-none transition-colors ${selectBg} ${theme.ring}`}>
      ...
      <div className={`flex-1 rounded-xl overflow-hidden flex flex-col relative border ${bgCard}`}>
        ...
        {filteredReports.length === 0 && !isLoading ? (
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
            ...
            <p className={`text-xs max-w-sm mx-auto ${textMuted}`}>Wait for students to generate and save their Weekly Progress Reports.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse min-w-max">
              <thead className={`sticky top-0 border-b z-10 ${bgHeader}`}>
                <tr>
                  ...
                  <th className={`py-3 px-4 text-[11px] font-bold uppercase tracking-wider ${textMuted}`}>Student</th>
                  ...
                  <tr key={report.id} className={`transition-colors group ${bgHover}`}>
                    ...
                    <td className="py-2.5 px-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className={`text-[10px] uppercase font-bold tracking-wider ${textMuted}`}>{department}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-2.5 px-4">
                      <div className="flex items-center gap-2">
                        ...
                        {/* Always display .docx to user even if backed by json */}
                    ...
                    <td className="py-2.5 px-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button onClick={() => {setReportToDelete(report); setIsDeleteModalOpen(true);}} className={`p-1.5 rounded-md transition-colors ml-1 ${btnDelete}`}>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl shadow-2xl w-full max-w-[300px] flex flex-col p-6 text-center items-center border border-white/10 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <h3 className={`text-lg font-bold mb-2 ${textMain}`}>Delete Document?</h3>
            <p className={`text-[12px] mb-6 leading-relaxed ${textMuted}`}>Remove <span className={`font-bold ${textMain}`}>{reportToDelete?.file_name}</span>? This will also remove the file from storage.</p>
