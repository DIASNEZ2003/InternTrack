...
function Settings() {
  ...
  const [showCurrentPassword, setShowCurrentPassword] = useState(false); // Controls the eye toggle for current password
  
  const fileInputRef = useRef(null);

  // GLOBAL DARK MODE LISTENER & TOGGLE
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        // Fetches all info, including raw_password
  ...
  const getThemeColors = (deptCode) => {
    switch (deptCode) {
      case 'BSIT': return { primary: 'bg-purple-600', hover: 'hover:bg-purple-500', text: 'text-purple-600', ring: 'focus:ring-purple-500', bgLight: 'bg-purple-500/20' };
  ...
  // --- DYNAMIC VARIABLES (Switches between Light and Dark mode) ---
  ...
  const handleAvatarUpload = async (event) => {
    try {
      ...
      if (!file) return;
      const fileName = `admin-${userId}-${Math.random()}.${file.name.split('.').pop()}`;
  ...
  const savePassword = async (e) => {
    ...
    try {
      ...
      // 2. Also save the raw password so the admin can view it later!
  ...
  return (
    <div className="h-full flex flex-col p-4 sm:p-8 animate-fade-in w-full overflow-y-auto custom-scrollbar">
      {feedback.message && (
      ...
      <div className="flex flex-col md:flex-row gap-8 mt-2">
        <div className="w-full md:w-[220px] shrink-0">
          <nav className="flex flex-row md:flex-col gap-1">
            <button onClick={() => setActiveTab('Profile')} className={`text-left px-3 py-2 text-[13px] font-bold rounded-lg transition-colors ${activeTab === 'Profile' ? tabActive : `text-gray-500 ${tabHover}`}`}>Public profile</button>
            <button onClick={() => setActiveTab('Appearance')} className={`text-left px-3 py-2 text-[13px] font-bold rounded-lg transition-colors ${activeTab === 'Appearance' ? tabActive : `text-gray-500 ${tabHover}`}`}>Appearance</button>
            <button onClick={() => setActiveTab('Security')} className={`text-left px-3 py-2 text-[13px] font-bold rounded-lg transition-colors ${activeTab === 'Security' ? tabActive : `text-gray-500 ${tabHover}`}`}>Account security</button>
          </nav>
        </div>
        ...
        <div className="flex-1 max-w-3xl">
          ...
          {activeTab === 'Profile' && (
            <div className="animate-fade-in">
              ...
              <div className="flex flex-col-reverse md:flex-row gap-8">
                <form onSubmit={saveProfile} className="flex-1 flex flex-col gap-5">
                  ...
                  <div>
                    <label className={`block text-[12px] font-bold mb-1.5 ${textMuted}`}>Department / Course</label>
                    <select name="department" value={profileData.department} onChange={handleProfileChange} className={`w-full px-3 py-2 rounded-md outline-none transition-colors ${bgInput} ${theme.ring}`}>
                  ...
                  <div className="pt-2"><button type="submit" disabled={isSaving} className={`px-4 py-2 rounded-md font-bold text-[12px] text-white transition-colors ${theme.primary}`}>{isSaving ? 'Saving...' : 'Update profile'}</button></div>
                </form>

                <div className="w-full md:w-[200px] shrink-0 flex flex-col">
                  <label className={`block text-[12px] font-bold mb-1.5 ${textMuted}`}>Profile picture</label>
                  <div className={`relative group inline-block rounded-full w-[200px] h-[200px] overflow-hidden border-4 ${borderDivider} ${isDarkMode ? 'bg-black/20' : 'bg-gray-100'}`}>
                    {profileData.avatar_url ? <img src={profileData.avatar_url} className="w-full h-full object-cover" /> : <div className={`w-full h-full flex items-center justify-center ${isDarkMode ? 'text-white' : theme.text}`}><svg className="w-24 h-24 opacity-60" fill="currentColor" viewBox="0 0 24 24"><path d="M12 12c2.21 0 4-1.79 4-4s-1.79-4-4-4-4 1.79-4 4 1.79 4 4 4zm0 2c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4z" /></svg></div>}
          ...
          {activeTab === 'Appearance' && (
            <div className="animate-fade-in">
              ...
              <div className={`flex items-center justify-between max-w-md p-4 rounded-xl border ${isDarkMode ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'}`}>
                <div>
                ...
                <button 
                  ...
                  className={`w-12 h-6 rounded-full flex items-center transition-colors px-1 ${isDarkMode ? theme.primary.split(' ')[0] : 'bg-gray-300'}`}
                >
            ...
            <div className="animate-fade-in">
              ...
              <div className="mb-6 max-w-md">
                ...
                <div className={`flex items-center justify-between px-4 py-2.5 rounded-lg border ${isDarkMode ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
                  <span className={`text-[13px] ${!profileData.raw_password ? 'italic opacity-70 text-amber-500 font-medium' : 'font-mono tracking-wide'} ${textMain}`}>
                    {showCurrentPassword ? (profileData.raw_password || 'Private Password') : '••••••••••••'}
                  </span>
                  <button 
                    type="button" 
                    onClick={() => setShowCurrentPassword(!showCurrentPassword)} 
                    className={`transition-colors p-1 rounded-md ${isDarkMode ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-200'}`}
                  >
                    {showCurrentPassword ? (
              ...
              <form onSubmit={savePassword} className="flex flex-col gap-5 max-w-md pt-4 border-t border-dashed border-gray-300/30">
                <div>
                  <label className={`block text-[12px] font-bold mb-1.5 ${textMuted}`}>Change to New Password</label>
                  <div className="relative">
                    <input type={showPassword ? "text" : "password"} name="newPassword" value={passwordData.newPassword} onChange={handlePasswordChange} required placeholder="Enter new password" className={`w-full px-3 py-2 pr-10 rounded-md outline-none transition-colors ${bgInput} ${theme.ring}`} />
                ...
                <div>
                  ...
                  <input type={showPassword ? "text" : "password"} name="confirmPassword" value={passwordData.confirmPassword} onChange={handlePasswordChange} required placeholder="Confirm new password" className={`w-full px-3 py-2 pr-10 rounded-md outline-none transition-colors ${bgInput} ${theme.ring}`} />
                </div>
                <div className="pt-2"><button type="submit" disabled={isSaving || !passwordData.newPassword} className={`px-4 py-2 rounded-md font-bold text-[12px] text-white transition-colors ${theme.primary} disabled:opacity-50`}>{isSaving ? 'Updating...' : 'Save New Password'}</button></div>
              </form>
