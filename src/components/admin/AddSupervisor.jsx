...
function AddSupervisor() {
  ...
  useEffect(() => { 
    fetchAdminAndSupervisors(); 
  }, []);

  const fetchAdminAndSupervisors = async () => {
    setIsLoading(true);
    try {
      ...
      const mappedSups = (supsData || []).map(sup => {
        ...
        return { ...sup, company_name: comp ? comp.name : 'Unassigned' };
      });

      setSupervisors(mappedSups);
  ...
  const filteredSupervisors = supervisors.filter(sup => (
    `${sup.first_name} ${sup.last_name}`.toLowerCase().includes(searchQuery.toLowerCase()) || 
    (sup.username || '').toLowerCase().includes(searchQuery.toLowerCase()) || 
    (sup.company_name || '').toLowerCase().includes(searchQuery.toLowerCase())
  ));
    ...
    <div className="h-full flex flex-col p-4 sm:p-6 animate-fade-in w-full relative overflow-hidden">
      ...
      <div className={`flex flex-col md:flex-row md:items-center justify-between w-full mb-4 gap-3 p-3 rounded-xl border shrink-0 ${bgCard}`}>
        <div className="relative flex-1 w-full max-w-sm">
          ...
          <input 
            type="text" 
            placeholder="Search supervisor or company..." 
            value={searchQuery} 
      ...
      <div className={`flex-1 rounded-xl overflow-hidden flex flex-col relative border ${bgCard}`}>
        {isLoading ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-6">
            <h3 className={`text-sm font-bold mb-1 ${textMain}`}>No supervisors found</h3>
          </div>
        ) : (
          <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col">
              ...
              <table className="w-full text-left border-collapse min-w-max">
                <thead className={`sticky top-0 border-b z-10 ${bgHeader}`}>
                  <tr>
                    ...
                    <th className={`py-3 px-4 text-[11px] font-bold uppercase tracking-wider ${textMuted}`}>Supervisor Profile</th>
                ...
                <tbody className={`divide-y ${isDarkMode ? 'divide-white/5' : 'divide-gray-100'}`}>
                  {filteredSupervisors.map((sup, index) => {
                    const displayPassword = sup.raw_password || sup.password || 'Private Password';
                    const isPrivate = displayPassword === 'Private Password';

                    return (
                      <tr key={sup.id} className={`transition-colors group ${bgHover}`}>
                        <td className={`py-2.5 px-4 text-center text-[11px] font-bold ${textMuted}`}>{index + 1}</td>
                        <td className="py-2.5 px-4">
                          <div className="flex items-center gap-3">
                            {sup.avatar_url ? (
                              <img src={sup.avatar_url} className={`w-8 h-8 rounded-lg object-cover shrink-0 shadow-sm border ${isDarkMode ? 'border-white/10' : 'border-gray-100'}`} />
                            ) : (
                            ...
                            <div>
                              <p className={`text-[13px] font-bold leading-tight ${textMain}`}>{sup.first_name} {sup.last_name}</p>
                              <p className={`text-[11px] uppercase ${textMuted}`}>{department}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-2.5 px-4">
                          <span className={`text-[12px] font-medium ${textMain}`}>{sup.company_name}</span>
                        </td>
                        <td className="py-2.5 px-4">
                        ...
                        <td className="py-2.5 px-4">
                          <div className="flex items-center gap-2">
                            <span className={`text-[12px] ${isPrivate ? 'font-medium italic opacity-70 text-amber-500' : 'font-mono tracking-wide'} px-2 py-1 rounded-md inline-block min-w-[80px] text-center border ${isDarkMode ? 'bg-black/40 border-white/5 text-gray-300' : 'bg-gray-100 border-transparent text-gray-700'}`}>
                              {visiblePasswords[sup.id] ? displayPassword : '••••••••'}
                            </span>
                            <button onClick={() => togglePasswordVisibility(sup.id)} className="text-gray-500 hover:text-gray-300 transition-colors p-1">
                              {visiblePasswords[sup.id] ? (
                        ...
                        <td className="py-2.5 px-4">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider border ${isDarkMode ? 'bg-amber-500/10 border-amber-500/20 text-amber-400' : 'bg-amber-50 border-amber-200 text-amber-700'}`}>
                            Supervisor
                          </span>
                        </td>
                        <td className="py-2.5 px-4 text-right">
                          <div className="flex items-center justify-end gap-1">
                            <button onClick={() => openModal('edit', sup)} className={`px-2.5 py-1.5 rounded-md text-[11px] font-bold transition-colors ${btnEdit}`}>
                              Edit
                            </button>
                            <button onClick={() => {setSupervisorToDelete(sup); setIsDeleteModalOpen(true);}} className={`p-1.5 rounded-md transition-colors ml-1 ${btnDelete}`}>
                              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="md:hidden flex flex-col gap-3 p-3 w-full">
              {filteredSupervisors.map((sup) => {
                ...
                return (
                  <div key={sup.id} className={`transition-all duration-300 rounded-xl border p-4 flex flex-col gap-3 relative shadow-sm ${isDarkMode ? 'bg-white/5 border-white/5 hover:bg-white/10' : 'bg-white border-gray-100 hover:bg-gray-50'}`}>
                    
                    {/* Header: Avatar + Info */}
                    <div className="flex items-center gap-3 w-full">
                      {sup.avatar_url ? (
                        <img src={sup.avatar_url} className={`w-10 h-10 rounded-lg object-cover shrink-0 shadow-sm border ${isDarkMode ? 'border-white/10' : 'border-gray-100'}`} />
                      ) : (
                      ...
                      <div className="flex flex-col flex-1 min-w-0">
                        <span className={`text-[13px] font-bold truncate ${textMain}`}>{sup.first_name} {sup.last_name}</span>
                        <span className={`text-[11px] uppercase ${textMuted}`}>{department}</span>
                      </div>
                      <div className={`text-[10px] font-bold px-2 py-1 rounded-md ${isDarkMode ? 'bg-amber-500/10 text-amber-400' : 'bg-amber-50 text-amber-700'}`}>
                        SUPERVISOR
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="flex flex-col">
                        ...
                        <span className={`text-[12px] font-medium truncate ${textMain}`}>{sup.company_name}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className={`text-[10px] uppercase font-bold tracking-wider mb-1 ${textMuted}`}>Username</span>
                        <span className={`text-[12px] font-medium truncate ${textMain}`}>{sup.username || 'N/A'}</span>
                      </div>
                    </div>
                    ...
                    <div className={`flex flex-wrap items-center justify-end gap-2 pt-2 mt-1 border-t ${isDarkMode ? 'border-white/10' : 'border-gray-100'}`}>
                      <button onClick={() => openModal('edit', sup)} className={`flex-1 justify-center inline-flex px-2.5 py-1.5 rounded-md text-[11px] font-bold transition-colors ${isDarkMode ? 'text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20' : 'text-gray-500 hover:text-amber-600 bg-gray-50 hover:bg-amber-50'}`}>
                        Edit Supervisor
                      </button>
                      <button onClick={() => {setSupervisorToDelete(sup); setIsDeleteModalOpen(true);}} className={`justify-center inline-flex p-1.5 rounded-md transition-colors ${isDarkMode ? 'text-gray-400 hover:text-red-400 bg-white/5 hover:bg-red-500/10' : 'text-gray-500 hover:text-red-600 bg-gray-50 hover:bg-red-50'}`}>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className={`rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden border border-white/10 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            <div className={`px-6 py-4 flex justify-between items-center border-b ${isDarkMode ? 'border-white/10 bg-black/40' : 'border-gray-100 bg-gray-50'}`}>
              <div>
                <h2 className={`text-lg font-bold tracking-tight ${textMain}`}>{modalMode === 'add' ? 'Register Supervisor' : 'Edit Supervisor'}</h2>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className={`rounded-2xl shadow-2xl w-full max-w-[300px] flex flex-col p-6 text-center items-center border border-white/10 ${isDarkMode ? 'bg-gray-900' : 'bg-white'}`}>
            ...
            <h3 className={`text-lg font-bold mb-2 ${textMain}`}>Remove Supervisor?</h3>
            <p className={`text-[12px] mb-6 leading-relaxed ${textMuted}`}>Delete <span className={`font-bold ${textMain}`}>{supervisorToDelete?.first_name}</span>?</p>
