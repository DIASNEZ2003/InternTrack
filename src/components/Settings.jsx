...
function Settings() {
  ...
  const handleAvatarUpload = async (event) => {
    try {
      ...
      if (!file) return;
      const fileName = `student-${userId}-${Math.random()}.${file.name.split('.').pop()}`;
  ...
  const saveProfile = async (e) => {
    ...
    setIsSaving(true);
    try {
      // NOTE: We only update name, department is locked for students
              ...
              <div className="flex flex-col-reverse md:flex-row gap-8">
                <form onSubmit={saveProfile} className="flex-1 flex flex-col gap-5">
                  ...
                  <div>
                    ...
                    {/* DISABLED INPUT FOR STUDENTS */}
                    <input type="text" value={profileData.department} disabled className={`w-full px-3 py-2 rounded-md outline-none transition-colors opacity-60 cursor-not-allowed ${bgInput}`} />
                    <p className={`text-[11px] mt-1.5 ${textMuted}`}>Students cannot change their department. Contact your admin if this is incorrect.</p>
                  </div>
          ...
          {activeTab === 'Appearance' && (
            <div className="animate-fade-in">
              ...
              <div className={`flex items-center justify-between max-w-md p-4 rounded-xl border ${isDarkMode ? 'border-white/10 bg-white/5' : 'border-gray-200 bg-gray-50'}`}>
                ...
                <button onClick={toggleDarkMode} className={`w-12 h-6 rounded-full flex items-center transition-colors px-1 ${isDarkMode ? theme.primary.split(' ')[0] : 'bg-gray-300'}`}>
            ...
            <div className="animate-fade-in">
              ...
              <div className="mb-6 max-w-md">
                ...
                <div className={`flex items-center justify-between px-4 py-2.5 rounded-lg border ${isDarkMode ? 'bg-black/40 border-white/10' : 'bg-gray-50 border-gray-200'}`}>
                  ...
                  <button type="button" onClick={() => setShowCurrentPassword(!showCurrentPassword)} className={`transition-colors p-1 rounded-md ${isDarkMode ? 'text-gray-400 hover:text-white hover:bg-white/5' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-200'}`}>
